...
class Question2025Seeder extends Seeder
{
    ...
    {
        // 1. Bảng mapping mức độ nhận thức (Chuyển chữ sang ID số)
        $levelMap = [
        ...
        $questionsData = [
            ...
            // --- CÂU HỎI CHÙM ĐÚNG/SAI ---
            [
                'meta' => '12B; Chung; D2; NLc',
                'content' => 'Tòa nhà của một công ty có một số phòng làm việc. Mỗi phòng có không quá 10 máy tính, được kết nối với nhau thông qua Switch tạo thành một mạng LAN. Công ty cần kết nối các mạng LAN này với nhau.',
                'children' => [
                    ['content' => 'Mạng cục bộ của công ty là mạng WAN.', 'level' => 'Biết', 'is_correct' => false],
                    ['content' => 'Có thể sử dụng Switch loại 16 cổng để kết nối các máy tính trong mỗi phòng làm việc.', 'level' => 'Hiểu', 'is_correct' => true],
                    ['content' => 'Để máy tính truy cập Internet chỉ cần lắp Modem mà không cần đăng ký với nhà cung cấp.', 'level' => 'Hiểu', 'is_correct' => false],
            ...
            [
                ...
                'children' => [
                    ['content' => 'Trường maKV là khóa ngoài của bảng KHUVUC.', 'level' => 'Biết', 'is_correct' => false],
                    ['content' => 'Nhóm hai trường maTinh và nam là khóa chính của bảng DANSO.', 'level' => 'Hiểu', 'is_correct' => true],
                    ['content' => 'Chỉ cần liên kết TINH và DANSO là kết xuất được tên tỉnh, tên khu vực, năm và dân số.', 'level' => 'Hiểu', 'is_correct' => false],
            ...
            [
                ...
                'children' => [
                    ...
                    ['content' => 'Không cần sử dụng các phương pháp tiền xử lý để làm sạch tập dữ liệu.', 'level' => 'H', 'is_correct' => false],
                    ['content' => 'Mô hình học máy phù hợp nhất là mô hình Học có giám sát.', 'level' => 'B', 'is_correct' => true],
                    ['content' => 'Chỉ cần sử dụng hệ thống dự đoán để biết chính xác mình có mắc Hội chứng hay không.', 'level' => 'VD', 'is_correct' => false],
                ]
            ],
            [
                'meta' => '11E; R-CS; D2; NLc',
                'content' => 'Cho hàm sắp xếp chèn (Insertion Sort) viết bằng Python/C++ (Dựa trên code trong đề).',
                'children' => [
                    ['content' => 'Mảng A biểu diễn cấu trúc dữ liệu ngăn xếp (Stack).', 'level' => 'B', 'is_correct' => false],
                    ['content' => 'Nếu mảng A đã sắp xếp tăng dần thì vòng lặp while (Dòng 5) không bao giờ thực hiện.', 'level' => 'H', 'is_correct' => true],
                    ['content' => 'Nếu n=6, A=(6,4,2,1,3,5), sau lần lặp i=2 (xét phần tử 2), mảng A là (2,4,6,1,3,5).', 'level' => 'VD', 'is_correct' => true],
                    ['content' => 'Nếu n=7, A=(8,6,4,2,3,5,7), hàm trả về giá trị trung vị là 6.5.', 'level' => 'H', 'is_correct' => false],
                ]
            ],
            [
                'meta' => '11E; R-ICT; D2; NLc',
                ...
                'children' => [
                    ...
                    ['content' => 'Các mục con CLB bắt buộc phải hiển thị ngay trên thanh điều hướng chính.', 'level' => 'H', 'is_correct' => false],
                    ['content' => 'Để xem video, phải vừa tải lên máy chủ vừa nhúng link YouTube.', 'level' => 'VD', 'is_correct' => false],
                ]
            ],
            [
                ...
                'children' => [
                    ...
                    ['content' => 'Khi tạo bảng DAUTU, thiết lập trường maDP làm khóa chính.', 'level' => 'B', 'is_correct' => false],
        ...
        foreach ($questionsData as $q) {
            ...
            $levelId = $levelMap[$rawLevel] ?? 2; // Default: Thông hiểu

            // Tìm Topic ID
            $topic = Topic::where('name', 'like', "%Chủ đề $topicCode%")->first();

            // Tạo Câu hỏi cha (ĐÃ XÓA user_id)
            $question = Question::create([
                ...
                'source' => 'thpt_2025',
                // 'user_id' => 1, // <--- Đã xóa dòng này
            ]);

            // Tạo Đáp án / Câu hỏi con
            if ($type == 'single_choice') {
                ...
                foreach ($q['children'] as $child) {
                    ...
                    // Tạo câu hỏi con (ĐÃ XÓA user_id)
